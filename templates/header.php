...
<header class="fixed top-0 left-0 w-full z-50 hover:bg-white transition duration-300">
  <div class="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto relative">
    ...
    <button id="menu-toggle" class="focus:outline-none z-50 <?= $isHome ? 'bg-white' : 'bg-white' ?> p-2">
      <svg id="menu-icon" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 <?= $isHome ? 'text-black' : 'text-gray-800' ?>" fill="none"
